<?php
/**
 * Portfolio | nucleus_portfolio
 *
 * @var array $atts    Shortcode attributes
 * @var mixed $content Shortcode content
 *
 * @author 8guild
 */

if ( ! defined( 'ABSPATH' ) ) {
	die;
}

/**
 * Filter the "nucleus_portfolio" default shortcode attributes
 *
 * @param array $atts Pairs of default attributes
 */
$a = shortcode_atts( apply_filters( 'nucleus_shortcode_portfolio_atts', array(
	'type'                 => 'standard', // standard | gap | no-gap
	'source'               => 'categories', // categories | ids
	'query_post__in'       => '', // list of IDs
	'query_categories'     => '', // list of term slugs
	'query_post__not_in'   => '',
	'query_posts_per_page' => 'all',
	'query_orderby'        => 'date',
	'query_order'          => 'DESC', // DESC | ASC
	'is_more'              => 'yes',
	'more_pos'             => 'center', // left | center | right
	'more_text'            => __( 'Load More', 'nucleus' ),
	'is_filters'           => 'no',
	'filters_pos'          => 'center',
	'filters_exclude'      => '', // list of term slugs
	'is_animation'         => 'disable',
	'animation_type'       => 'top',
	'animation_delay'      => 0,
	'animation_easing'     => 'none',
	'class'                => '',
) ), $atts );

$post_type    = 'nucleus_portfolio';
$tax          = 'nucleus_portfolio_category';
$type         = esc_attr( $a['type'] );
$unique_id    = nucleus_get_unique_id( 'portfolio-' );
$source       = sanitize_key( $a['source'] );
$is_filters   = ( 'yes' === $a['is_filters'] );
$is_more      = ( 'yes' === $a['is_more'] );
$is_all       = ( 'all' === strtolower( $a['query_posts_per_page'] ) );
$is_animation = ( 'enable' === $a['is_animation'] );
$animation    = nucleus_parse_array( $a, 'animation_' );

$query_default = array(
	'post_type'   => $post_type,
	'post_status' => 'publish',
);

$query_args = nucleus_parse_array( $a, 'query_' );
$query_args = array_merge( $query_default, $query_args );
$query_args = nucleus_query_build( $query_args, function( $query ) use ( $tax, $source ) {
	// build a tax_query if getting by categories
	// @see WP_Query
	if ( array_key_exists( 'categories', $query ) ) {
		$categories = $query['categories'];
		unset( $query['categories'] );

		$query['tax_query'] = nucleus_query_single_tax( $categories, $tax );
	}

	// always fetch all posts if getting by IDs
	if ( 'ids' === $source ) {
		$query['posts_per_page'] = - 1;
	}

	return $query;
} );

$query = new WP_Query( $query_args );

if ( $query->have_posts() ) :
	// unique IDs using in JS
	$grid_id    = $unique_id . '-grid';
	$filters_id = $unique_id . '-filters';

	// if animation enabled
	if ( $is_animation ) {
		echo '<div class="', nucleus_get_animation_class( $is_animation, $animation ), '">';
	}

	// build a filters
	if ( $is_filters ) {
		$filter_cats  = nucleus_get_filter_cats( $tax, $a['filters_exclude'] );
		$filter_class = nucleus_get_class_set( array(
			'nav-filters',
			'space-bottom-2x',
			'portfolio-categories',
			'nucleus-isotope-filter',
		) );

		echo nucleus_get_filter_markup( $filter_cats, array(
			'id'       => esc_attr( $filters_id ),
			'class'    => esc_attr( $filter_class ),
			'grid_id'  => esc_attr( $grid_id ),
			'show_all' => __( 'Show All', 'nucleus' ),
			'position' => sanitize_key( $a['filters_pos'] ),
		) );

		unset( $filter_cats, $filter_class );
	}

	// build a grid
	$grid_class = nucleus_get_class_set( array(
		'grid',
		'isotope-grid',
		'col-3',
		'filter-grid',
		$type,
		'nucleus-isotope',
		$a['class'],
	) );

	$grid_atts = array(
		'id'    => esc_attr( $grid_id ),
		'class' => esc_attr( $grid_class ),
	);
	?>
	<div <?php echo nucleus_get_html_attr( $grid_atts ); ?>>
		<div class="grid-sizer"></div>
		<div class="gutter-sizer"></div>

		<?php
		while ( $query->have_posts() ) :
			$query->the_post();
			get_template_part( 'template-parts/portfolio', $type );
		endwhile;
		?>
		<div class="preloader">
			<img alt="<?php esc_html_e( 'Portfolio Preloader', 'nucleus' ); ?>"
			     src="data:image/gif;base64,">
		</div>
	</div>
	<?php
	unset( $grid_class, $grid_atts );

	/*
	 * Load More
	 *
	 * Show load more only when user perform a request by categories
	 * and limiting the number of posts. In case if user try to load
	 * "all" posts, or perform a request by posts - Load More button
	 * won't display.
	 */
	if ( $is_more
	     && false === $is_all
	     && 'categories' === $source
	     && (int) $query->found_posts > $query_args['posts_per_page']
	) {
		$more_pos   = 'text-' . $a['more_pos'];
		$more_class = nucleus_get_class_set( array(
			'btn',
			'btn-default',
			'btn-ghost',
			'waves-effect',
			'nucleus-portfolio-more',
		) );

		$more_attr = array(
			'href'           => '#',
			'class'          => esc_attr( $more_class ),
			'data-query'     => nucleus_query_encode( $query_args ),
			'data-page'      => 2,
			'data-max-pages' => (int) $query->max_num_pages,
			'data-type'      => $type,
			'data-grid-id'   => $grid_id,
			'data-filter-id' => $is_filters ? $filters_id : '',
		);

		$more_text = wp_kses( $a['more_text'], array(
			'i' => array( 'class' => true ),
		) );

		echo '<div class="padding-top ', esc_attr( $more_pos ), '">';
		echo nucleus_get_tag( 'a', $more_attr, $more_text );
		echo '</div>';
		unset( $more_pos, $more_class, $more_attr, $more_text );
	}

	if ( $is_animation ) {
		echo '</div>'; // close div.scrollReveal
	}

endif; // have_posts
wp_reset_postdata();
