...
class Dependencies {
	...
	public function handle( $layout, $values ) {
		...
		foreach ( $fields as $key => $field ) {
			...
			// skip fields without dependencies
		...
		// and mark layout as already tested
		$layout->set_flag( 'dependencies', true );
	...
	public function compare( $master_value, $operator, $compare_value ) {
		...
		switch ( $operator ) {
			...
			case 'in_array':
				...
				$result = in_array( $master_value, (array) $compare_value );
				break;
			...
			default:
				 ...
				 */
				$result = apply_filters( 'equip/dependencies/compare', $result, $master_value, $operator, $compare_value );
				break;
		}
